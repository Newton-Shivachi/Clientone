...
def add_sales_layers_entry(date, product, sales, quantities, payment_mode):
    ...
    add_or_update_inventory_layers(date, product, -sum(quantities)) 
...
layers_action = st.radio("Select action", ["Record ", "Mature Birds Analysis"])
if layers_action =="Mature Birds Analysis":
    ...
    if st.button("Analyse profit"):    
        ...
        x=merged_sales_expenses['Bill'].sum()
        w=merged_sales_expenses['Profit'].sum()
        y=merged_sales_expenses['Sales'].sum()
        col11,col12,col13=st.columns((3))
        with col11:
            with st.expander('view mature birds profit'):
                st.write(w)
        with col12:
            with st.expander('view mature birds sales'):
                st.write(y)
        with col13:
            with st.expander('view mature birds bill'):
                st.write(x)
    ...
    products = inventory_data_layers['Product'].tolist()  
    ...
    payment_mode = st.selectbox('Payment Mode',['Cash','M-Pesa','Bank Account','Others'])

    if st.button('Add Entry'):
        if sales_date and selected_products and sales and sales_quantity and payment_mode: 
            for product,sales, quantity in zip(selected_products, map(str, sales.split(',')) ,map(int, sales_quantity.split(','))):
                ...
                add_sales_layers_entry(sales_date, product, sales_list, [quantity], payment_mode) 
    ...
    if inventory_action == "View Inventory":
        ...
        if st.button('Add/Update Inventory'):
            if inventory_date and inventory_product and inventory_quantity: 
    ...
    if st.button('Add Expense'):
        if expense_date and expense_expense and expense_bill:  
    ...
    if st.button('Add Egg Production'):
        if egg_date and bird_number and number_of_eggs: 
...
if meat_action =="Analysis":
    ...
    if st.button('add Entry'):
        if sales_date and selected_products and sales and sales_quantity and payment_mode:  
            ...
            add_sales_meat_entry(sales_date, selected_products, sales, sales_quantity, payment_mode)  
            st.sidebar.success('Entry Added successfully!')
